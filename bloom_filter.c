...
static uint64_t * hashing_by_default(int num_hashes, const char * string); 
...
int add_string(bloom_filter * bloomFilter, const char * string){
    ...
    uint64_t * hash_array = bloom_filter_calculate_hashes(bloomFilter,string, bloomFilter->number_of_hashing_func); 
...
// 6 all okay 
int is_present_bloom_filter(bloom_filter * bloomFilter, const char * string)
{
    uint64_t * hash_array = bloom_filter_calculate_hashes(bloomFilter, string, bloomFilter->number_of_hashing_func);
...
// 5 
uint64_t * bloom_filter_calculate_hashes(bloom_filter * bloomFilter, const char * string, unsigned int num_hashes)
{
    return bloomFilter->hashing_function(num_hashes, string); 
}
